...
.xh-list-card-facts{box-sizing:border-box;display:block;flex:0 0 auto;align-self:stretch;clear:both;width:100%;min-width:0;margin:8px 0 0;padding:7px;color:#111;background:#fff;border:1px solid #111;border-radius:0;font-family:"Microsoft YaHei","PingFang SC","Segoe UI",sans-serif;font-size:11px;line-height:1.25}.xh-list-card-host{overflow:visible!important}.xh-list-card-host-vertical{display:flex!important;flex-direction:column!important;align-items:stretch!important}.xh-list-card-grid{display:grid;grid-template-columns:minmax(0,1fr) 54px;gap:6px 8px}.xh-list-card-facts[data-density=mini]{padding:6px;font-size:10px}.xh-list-card-facts[data-density=mini] .xh-list-card-grid{grid-template-columns:minmax(0,1fr) 38px;gap:5px}.xh-list-card-grid>div{min-width:0}.xh-list-card-grid span,.xh-list-card-row span{display:block;color:#737373;font-size:9px;font-weight:850}.xh-list-card-grid strong,.xh-list-card-row strong{display:block;margin-top:2px;color:#111;font-size:11px;font-weight:900;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.xh-list-card-spinner{display:inline-block;width:9px;height:9px;margin-right:4px;vertical-align:-1px;border:1px solid #d4d4d4;border-top-color:#111;border-radius:999px;animation:xh-list-card-spin .72s linear infinite}@keyframes xh-list-card-spin{to{transform:rotate(360deg)}}.xh-list-card-row-muted strong{color:#525252}.xh-list-card-row-error strong{color:#b91c1c}
.xh-list-card-shell,.xh-list-card-host{overflow:visible!important;height:auto!important;min-height:0!important;contain:none!important}.xh-list-card-host{position:relative!important}.xh-list-card-host-vertical{display:flex!important;flex-direction:row!important;flex-wrap:wrap!important;align-items:stretch!important}.xh-list-card-dock{box-sizing:border-box;display:block;flex:0 0 100%;width:100%;min-width:0;margin-top:8px;order:2147483647;grid-column:1/-1;align-self:stretch;position:relative;z-index:2}.xh-list-card-dock>.xh-list-card-facts{margin:0}
...
function getListingCardDock(card) {
  const existingDock = Array.from(card.children).find(
    (child) => child instanceof HTMLElement && child.classList.contains(LIST_CARD_DOCK_CLASS),
  );
  if (existingDock) {
    return existingDock;
  }
...
function scheduleAutoSalePriceSync({ recalculate = true } = {}) {
  ...
  if (applyAutoSalePriceFromPage()) {
    ...
    if (shouldRecalculate && state.settings?.extensionToken && state.settings?.defaultStoreId && !state.loading) {
      void requestPreview();
    }
    return;
  }
...
function scanListingProductCards() {
  ...
  for (const seed of seeds) {
    ...
    const card = findListingCardLayoutHost(matchedCard, plid);
