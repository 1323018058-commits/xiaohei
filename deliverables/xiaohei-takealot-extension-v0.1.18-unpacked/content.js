...
#xh-erp-panel[data-placement=inline],#xh-erp-panel[data-placement=floating]{position:static;width:min(318px,100%);max-width:318px;z-index:auto}
...
function mountPanel(panel) {
  ...
  if (sidebarTarget?.reference?.parentElement) {
    panel.dataset.placement = "sidebar";
    const { reference, mode } = sidebarTarget;
    if (mode === "after") {
      if (panel.previousElementSibling !== reference || panel.parentElement !== reference.parentElement) {
        reference.insertAdjacentElement("afterend", panel);
      }
    } else if (panel.parentElement !== reference.parentElement || panel.nextElementSibling !== reference) {
  ...
  if (reference?.parentElement && isRightRailElement(reference)) {
    panel.dataset.placement = "sidebar";
...
function buildConnectedView() {
  ...
  const panelNote = !state.preview
    ...
    : hasCompleteFacts
      ...
      : "ERP 接口未返回完整重量/尺寸，已阻止上架；请确认该 PLID 已同步上架规格。";
  ...
  const draftSalePrice = firstPositiveText(state.pricingDraft.salePriceZar, pricing?.sale_price_zar);
...
async function requestPreview() {
  ...
  try {
    ...
    if (listingPreviewHasWeight(state.preview) && listingPreviewHasDimensions(state.preview)) {
      ...
      setHint("已实时请求 ERP/Takealot 接口，但接口未返回完整重量尺寸。", "error");
    }
