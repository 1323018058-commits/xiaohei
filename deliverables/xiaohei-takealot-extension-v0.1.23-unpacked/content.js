...
function mountListingCardBox(card, box) {
  ...
  if (reference?.parentElement && card.contains(reference)) {
    reference.insertAdjacentElement("beforebegin", box);
    return;
  }
...
function scheduleAutoSalePriceSync({ recalculate = true } = {}) {
  ...
  if (applyAutoSalePriceFromPage()) {
    ...
    if (recalculate && state.settings?.extensionToken && state.settings?.defaultStoreId && !state.loading) {
      void requestPreview();
    }
    return;
  }
  ...
  state.autoSalePriceTimer = window.setTimeout(() => {
    ...
    if (state.plid === state.autoSalePricePlid) {
      scheduleAutoSalePriceSync({ recalculate });
    }
  }, delay);
}
...
function scanListingProductCards() {
  ...
  for (const seed of seeds) {
    ...
    const card = promoteListingCardHost(
      findListingProductCard(seed.node) || findFallbackListingCardHost(seed.node),
    );
